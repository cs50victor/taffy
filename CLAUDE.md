...
This repository is an independent personal derivative of cmux. For Taffy releases use `scripts/build-taffy.sh <version>`, the public `cs50victor/taffy` releases, and the existing `cs50victor/homebrew-tap` cask. Use ad-hoc signing; do not use upstream credentials, fleet services, release tags, or publication workflows. Upstream workflows are retained for reference under `.github/upstream-workflows` and do not run.

The Taffy Release build is isolated by its bundle identifier, socket, and build directory and can run beside cmux. Internal target names remain cmux. Do not replace or quit the user's cmux installation while testing Taffy. Follow the relevant contributor guidance below for code changes, with the Taffy release workflow taking precedence over upstream-only procedures.
