...
struct CLISocketPathResolver {
    ...
    static let legacyDefaultSocketPath = "/tmp/taffy.sock"
    private static let fallbackSocketPath = "/tmp/taffy-debug.sock"
    private static let nightlySocketPath = "/tmp/taffy-nightly.sock"
    private static let stagingSocketPath = "/tmp/taffy-staging.sock"
    ...
    private func resolvedStableImplicitDefaultPaths() -> [String] {
        Self.dedupe([
            ...
            stateDirectory.appendingPathComponent("taffy-\(currentUserID).sock", isDirectory: false).path,
