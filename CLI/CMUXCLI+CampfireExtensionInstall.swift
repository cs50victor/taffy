        ...
        if !existing.isEmpty, !existing.contains(Self.campfireExtensionMarker) {
            throw CLIError(message: String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "%@ exists and is not a Taffy extension; leaving it alone"
                ),
                extensionURL.path
            ))
        }
    ...
    func uninstallCampfireExtensionHooks(_ _: AgentHookDef) throws {
        ...
        guard fm.fileExists(atPath: extensionURL.path) else {
            print(String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "No Campfire Taffy extension found at %@"
                ),
                extensionURL.path
            ))
            return
        ...
        guard existing.contains(Self.campfireExtensionMarker) else {
            print(String.localizedStringWithFormat(
                String(
                    ...
                    defaultValue: "Refusing to remove %@: missing Taffy marker"
                ),
                extensionURL.path
            ))
            return
        ...
        print(String.localizedStringWithFormat(
            String(
                ...
                defaultValue: "Removed Campfire Taffy extension from %@"
            ),
            extensionURL.path
        ))
    }
