    ...
    private func removeKimiHooks(
        ...
        print(String.localizedStringWithFormat(
            String(
                ...
                defaultValue: "Removed Kimi Code taffy hooks from %@"
            ),
            configURL.path
        ))
        return true
    ...
    private func reportKimiSecondaryRefreshWarning(
        ...
        let warning = String.localizedStringWithFormat(
            String(
                ...
                defaultValue: "Warning: taffy hooks are installed at %@, but Taffy could not refresh its existing hook block in %@. Check that path and re-run `taffy hooks setup kimi` to finish the update."
            ),
    ...
    private func reportKimiSecondaryUninstallWarning(secondaryConfigURL: URL) {
        let warning = String.localizedStringWithFormat(
            String(
                ...
                defaultValue: "Warning: Taffy could not remove its hook block from %@. Check that path and re-run `taffy hooks uninstall kimi` to finish cleanup."
            ),
            secondaryConfigURL.path
        )
        cliWriteStderr(warning + "\n")
