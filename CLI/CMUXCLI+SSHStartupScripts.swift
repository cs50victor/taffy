...
extension CMUXCLI {
    ...
    func sshAskpassExecShellScript(passwordCredential: String) -> String {
        ...
        return [
            ...
            "  puts stderr {\\n[taffy] Cloud VM SSH credential was rejected; reconnecting.}",
            "  catch {close}",
            "  catch {wait}",
            "  exit 255",
            "}",
            ...
            "  }",
            "  timeout {",
            "    puts stderr {\\n[taffy] Cloud VM SSH credential prompt timed out; reconnecting.}",
            "    exit 255",
            "  }",
            "  eof { set status [wait]; exit [lindex $status 3] }",
            "}",
    ...
    private func buildSSHTerminalSessionLaunchingShellCommand() -> String {
        ...
        return [
            ...
            "&& [ -n \"${CMUX_WORKSPACE_ID:-}\" ]",
            "&& [ -n \"${CMUX_SURFACE_ID:-}\" ]; then",
            "CMUXTERM_CLI_RESPONSE_TIMEOUT_SEC=2 taffy --socket \"${CMUX_SOCKET_PATH}\" \(arguments) >/dev/null 2>&1;",
            "else",
            "false;",
            "fi",
        ].joined(separator: " ")
