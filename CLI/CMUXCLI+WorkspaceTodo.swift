...
extension CMUXCLI {
    ...
    private func workspaceTodoItemSelectorParams(_ raw: String) throws -> [String: Any] {
        ...
        throw CLIError(message: "Invalid todo item: \(trimmed) (expected an item UUID or a 1-based index from `taffy todo list`)")
    }
    ...
    func runWorkspaceStatusCommand(
        ...
        case "set":
            guard rest.count >= 2 else {
                throw CLIError(message: "Usage: taffy workspace status set <todo|working|needs-attention|review|done|auto|none>")
            }
        ...
        case "cycle":
            ...
            printWorkspaceStatusPayload(payload, jsonOutput: jsonOutput, idFormat: idFormat)
        case .some(let sub):
            throw CLIError(message: "Unknown workspace status subcommand: \(sub). Try: taffy workspace status [set <lane|auto> | cycle]")
        }
    }
    ...
    func runTodoNamespace(
        ...
        case "add":
            ...
            guard !text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
                throw CLIError(message: "Usage: taffy todo add \"text\" [--state <pending|in-progress|completed>] [--origin <user|agent>]")
            }
            addParams["text"] = text
        ...
        case "check", "uncheck", "start":
            guard let selector = rest.first(where: { !$0.hasPrefix("--") }) else {
                throw CLIError(message: "Usage: taffy todo \(sub) <index|id>")
            }
        ...
        case "edit":
            ...
            guard positional.count >= 2 else {
                throw CLIError(message: "Usage: taffy todo edit <index|id> \"new text\"")
            }
        ...
        case "rm", "remove":
            guard let selector = rest.first(where: { !$0.hasPrefix("--") }) else {
                throw CLIError(message: "Usage: taffy todo rm <index|id>")
            }
        ...
        case "move", "mv":
            ...
            guard positional.count >= 2, let newIndex = Int(positional[1]), newIndex >= 1 else {
                throw CLIError(message: "Usage: taffy todo move <index|id> <newIndex> (newIndex is 1-based)")
            }
    ...
    private func workspaceTodoSetItemsArgument(rest: [String]) throws -> [[String: Any]] {
        ...
        if let inline = rest.first(where: { !$0.hasPrefix("--") }) {
            ...
            guard isatty(STDIN_FILENO) == 0 else {
                throw CLIError(message: "Usage: taffy todo set '[{\"text\":\"...\",\"state\":\"pending\"}]' (or pipe the JSON on stdin)")
            }
            var data = Data()
    ...
    Usage: taffy workspace <subcommand> [flags]
    ...
    Subcommands:
      ...
      group <subcommand>      Workspace group operations (see taffy workspace-group --help)
    ...
    Examples:
      taffy workspace list --json
      taffy workspace create --name Build --cwd ~/projects/myapp
      taffy workspace env workspace:3 --mask
      taffy workspace close workspace:3
      taffy workspace reconnect
      taffy workspace disconnect --workspace workspace:3
    """)

    static let workspaceStatusUsage = String(localized: "cli.workspace.status.usage", defaultValue: """
    Usage: taffy workspace status [set <lane|auto> | cycle] [--workspace <id|ref|index>] [--window <id|ref|index>] [--json]
    ...
    Examples:
      taffy workspace status
      taffy workspace status set review
      taffy workspace status cycle
      taffy workspace status set auto --workspace workspace:2
    """)

    static let todoUsage = String(localized: "cli.todo.usage", defaultValue: """
    Usage: taffy todo <subcommand> [--workspace <id|ref|index>] [--window <id|ref|index>] [--json]
    ...
    <index> is the 1-based number printed by `taffy todo list`; <id> is the
    item UUID from `taffy todo list --json`.

    Examples:
      taffy todo add "write regression test"
      taffy todo list
      taffy todo check 1
      taffy todo start 2 --workspace workspace:3
      my-plan-tool --json | taffy todo set
      taffy todo open

    See also: taffy workspace status
    """)
}
