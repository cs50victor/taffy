...
/// the cmux app (bundle id `com.cs50victor.taffy`) and the standalone `cmux` CLI
...
///
/// This directory therefore resolves to `~/.local/state/taffy`, a plain dotfolder
/// macOS does **not** treat as protected app data. It is the sibling of the
/// existing `~/.local/state/taffy/crash` breadcrumb directory.
///
/// ```swift
...
/// let socket = CmuxStateDirectory.url(homeDirectory: home).appendingPathComponent("taffy.sock")
/// ```
public enum CmuxStateDirectory {
    ...
    public static let directoryName = "taffy"

    /// The cmux state directory: `<home>/.local/state/taffy`.
    ///
    ...
    /// (`~/Library/Application Support/taffy`).
    ///
