...
public enum SocketPathMarkerFiles {
    ...
    public static let stableBundleIdentifier = "com.cs50victor.taffy"
    public static let stableMarkerFileName = "last-socket-path"
    public static let stableTmpPath = "/tmp/taffy-last-socket-path"
    public static let nightlyBundleIdentifier = "com.cs50victor.taffy.nightly"
    public static let stagingBundleIdentifier = "com.cs50victor.taffy.staging"
    public static let defaultBaseDebugBundleIdentifier = "com.cs50victor.taffy.debug"
    public static let defaultDebugSocketPath = "/tmp/taffy-debug.sock"
    public static let defaultNightlySocketPath = "/tmp/taffy-nightly.sock"
    public static let defaultStagingSocketPath = "/tmp/taffy-staging.sock"
    ...
    public static func defaultSocketPath(
        ...
        case .nightly(let slug):
            if let slug {
                return "/tmp/taffy-nightly-\(slug).sock"
            }
            return nightlySocketPath
        case .staging(let slug):
            if let slug {
                return "/tmp/taffy-staging-\(slug).sock"
            }
            return stagingSocketPath
        case .dev(let slug):
            if let slug {
                return "/tmp/taffy-debug-\(slug).sock"
            }
