...
public enum SocketPathVariant: Equatable, Sendable {
    ...
    public var tmpPath: String {
        ...
        case .nightly(let slug):
            if let slug = Self.sanitizedSlug(slug) {
                return "/tmp/taffy-nightly-\(slug)-last-socket-path"
            }
            return "/tmp/taffy-nightly-last-socket-path"
        case .staging(let slug):
            if let slug = Self.sanitizedSlug(slug) {
                return "/tmp/taffy-staging-\(slug)-last-socket-path"
            }
            return "/tmp/taffy-staging-last-socket-path"
        case .dev(let slug):
            if let slug = Self.sanitizedSlug(slug) {
                return "/tmp/taffy-dev-\(slug)-last-socket-path"
            }
            return "/tmp/taffy-dev-last-socket-path"
        }
    }
