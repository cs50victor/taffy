        ...
        guard let simctlService = simctlPrivacyService(for: service) else {
            throw SimulatorControlError(
                ...
                message: String(
                    ...
                    defaultValue: "The active Xcode does not expose \(service.rawValue) through simctl; Taffy will not mutate private TCC or BulletinBoard stores in the host process."
                )
            )
        }
