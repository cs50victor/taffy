    ...
    var description: String {
        ...
        case let .missingCodexTokens(openAIAPIKeyPresent):
            if openAIAPIKeyPresent {
                return "Codex auth contains an OpenAI API key but no OAuth tokens. Use `Taffy ai-accounts upload openai-key` instead."
            }
        ...
        case let .missingAPIKey(provider):
            ...
            return "\(provider.displayName) upload requires `--key <value>` or \(envName) in the Taffy app environment."
