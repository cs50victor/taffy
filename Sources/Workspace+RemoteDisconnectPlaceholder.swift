...
extension Workspace {
    ...
    nonisolated static func remoteDisconnectPlaceholderScript(
        ...
        let endedLineFormat = String(
            ...
            defaultValue: "[Taffy] remote session disconnected: %s"
        )
        let reconnectLine = String(
            localized: "remote.disconnectBanner.reconnectHint",
            defaultValue: "[Taffy] Press Enter to reconnect. This terminal will stay disconnected until then."
        )
        let reconnectUnavailableLine = String(
            localized: "remote.disconnectBanner.reconnectUnavailableHint",
            defaultValue: "[Taffy] Reconnect this workspace from the sidebar or by running the original Taffy remote command again."
        )
